function [results, scp_log] = run_method_M6_gcs_scp(config_name, p)
% run_method_M6_gcs_scp.m  — M6: GCS-Warm-Started SCP
% Identical to M5 except: GCS initialisation + larger initial trust radii.
% =========================================================================

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  M6: GCS-WARM-STARTED SCP  —  %s\n', upper(config_name));
fprintf('═══════════════════════════════════════════════════════════════\n');
...
%% ── SCP parameters (same as M5, except trust radii) ─────────────────────
scp.M               = 60;
scp.max_iters       = 50;
scp.convergence_tol = 1e-3;

scp.trust_radius       = 1.0;
scp.trust_radius_u     = 8.0;
...
scp.w_slack     = 1e4;
scp.w_collision = 1e8;
scp.collision_margin = 0.12;

fprintf('  M=%d, max_iters=%d, rho=%.2f (GCS), rho_u=%.1f (GCS)\n', ...
    scp.M, scp.max_iters, scp.trust_radius, scp.trust_radius_u);

%% ── Initialise from GCS ──────────────────────────────────────────────────
fprintf('\n  Initialising from GCS...\n');
gcs_tic = tic;
try
    [x_bar, u_bar, t_f_bar] = scp_initialize_trajectory(p, scp, 'gcs');
catch ME
    fprintf('  ✗ GCS init failed: %s\n', ME.message);
    results = make_empty_result('M6 (GCS SCP)', config_name, ME.message);
    scp_log = struct([]);
    return;
end
t_gcs = toc(gcs_tic);
...
fprintf('  GCS init (%.2fs): miss=%.1f cm, J=%.1f, collisions=%d, t_f=%.2f\n', ...
    t_gcs, ci_old.miss_cm, J_old, ci_old.n_collisions, t_f_bar);

%% ── SCP loop (identical logic to M5) ────────────────────────────────────
scp_log = struct([]);
n_reject = 0;
total_tic = tic;
...
for iter = 1:scp.max_iters
    ...
    if isempty(z_sol)
        ...
        n_reject = n_reject + 1;
        if n_reject > 10, fprintf('  ⚠  Too many rejects\n'); break; end
        continue;
    end
    ...
    for ai = 1:length(alphas)
        ...
        if J_try < J_old - 1e-10
            x_new = x_try; u_new = u_try; t_f_new = t_f_try;
            J_new = J_try; ci_new = ci_try;
    ...
    if accepted_alpha > 0
        ...
        x_bar = x_new; u_bar = u_new; t_f_bar = t_f_new; J_old = J_new;
        if J_new < J_best
            J_best = J_new; x_best = x_new; u_best = u_new; t_f_best = t_f_new;
        end

        scp.trust_radius = rho_new; scp.trust_radius_u = rho_u_new;
    ...
    entry.iter = iter; entry.J_total = J_old; entry.miss_cm = ci_old.miss_cm;
    entry.dx_max = dx_max; entry.rho = scp.trust_radius;
    entry.accepted = (accepted_alpha > 0); entry.alpha = accepted_alpha;
...
t_optim = toc(total_tic);
t_total = t_gcs + t_optim;
...
fprintf('\n  ─── M6 FINAL: miss=%.1f cm, t=%.1fs (GCS:%.1f+opt:%.1f), iters=%d ───\n\n', ...
    results.miss_distance, results.t_total, t_gcs, t_optim, results.iterations);
end
...
function [n_arm, n_flt] = count_collisions(x_bar, t_f_bar, obstacles, p, scp)
    ...
    for k = 1:scp.M
        ...
        for lnk = 1:N
            ...
            [c,~]=check_collision(qr,obstacles,p); if c, n_arm=n_arm+1; end
        end
        alpha=sum(q_k); R=[cos(alpha) -sin(alpha);sin(alpha) cos(alpha)];
        op.type='object'; op.pos=ee_k+R*p.obj.r_gc; op.theta=alpha;
        [c,~]=check_collision(op,obstacles,p); if c, n_arm=n_arm+1; end
    end
    n_flt = 0;
    rc = release_condition(x_bar(1:N,end), x_bar(N+1:end,end), p);
    if ~isnan(rc.t_land)
        for ti = linspace(0, rc.t_land, 50)
            ...
            [c,~]=check_collision(bq,obstacles,p); if c, n_flt=n_flt+1; end
        end
    end
end
